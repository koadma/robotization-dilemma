...
public:
  ...
  Eqnsys getEquations(bool b) {//approximate
    ...
    if (!b) { //Parametric curve
      ...
      resh.eqns['a'] = Equation<longDouble>({ { acc.x / 2.0f, "tt" },{ vel.x, "t" } ,{ pos.x, "" },{ -1, "a" } });
      resh.eqns['b'] = Equation<longDouble>({ { acc.y / 2.0f, "tt" },{ vel.y, "t" } ,{ pos.y, "" },{ -1, "b" } });
      resh.eqns['c'] = Equation<longDouble>({ { acc.z / 2.0f, "tt" },{ vel.z, "t" } ,{ pos.z, "" },{ -1, "c" } });


      res.eqns['t'] =
        Equation<longDouble>({ { 1, "x" },{ -1, "a" } }) * Equation<longDouble>({ { 1, "x" },{ -1, "a" } }) +
        Equation<longDouble>({ { 1, "y" },{ -1, "b" } }) * Equation<longDouble>({ { 1, "y" },{ -1, "b" } }) +
        Equation<longDouble>({ { 1, "z" },{ -1, "c" } }) * Equation<longDouble>({ { 1, "z" },{ -1, "c" } }) +
        Equation<longDouble>({ { -radius*radius, "" } });
      res.eqns['t'].substitute(resh.eqns['a'], 'a');
      res.eqns['t'].substitute(resh.eqns['b'], 'b');
      res.eqns['t'].substitute(resh.eqns['c'], 'c');
    }
    else { //Moving sphere
      res.eqns['x'] = Equation<longDouble>({ { acc.x / 2.0f, "tt" },{ vel.x, "t" } ,{ pos.x, "" },{ -1, "x" } });
      res.eqns['y'] = Equation<longDouble>({ { acc.y / 2.0f, "tt" },{ vel.y, "t" } ,{ pos.y, "" },{ -1, "y" } });
      res.eqns['z'] = Equation<longDouble>({ { acc.z / 2.0f, "tt" },{ vel.z, "t" } ,{ pos.z, "" },{ -1, "z" } });
    }
    return res;
  }
